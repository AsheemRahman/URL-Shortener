...
class AuthController implements IAuthController {
    ...
    async refreshToken(req: Request, res: Response): Promise<void> {
        try {
            ...
            res.cookie("accessToken", newAccessToken, { httpOnly: true, secure: isProduction, domain: process.env.COOKIE_DOMAIN, sameSite: isProduction ? "none" : "lax", maxAge: parseInt(process.env.ACCESS_TOKEN_MAX_AGE || "1440000") });
    ...
    async logout(req: Request, res: Response): Promise<void> {
        try {
            const isProduction = process.env.NODE_ENV === "production";
            res.clearCookie("accessToken", { httpOnly: true, secure: isProduction, domain: process.env.COOKIE_DOMAIN, sameSite: isProduction ? "none" : "lax", });
            res.clearCookie("refreshToken", { httpOnly: true, secure: isProduction, domain: process.env.COOKIE_DOMAIN, sameSite: isProduction ? "none" : "lax", });
